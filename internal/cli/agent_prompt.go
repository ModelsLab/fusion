...
func buildOptimizeRunPrompt(session *optimize.Session) string {
	...
	lines := []string{
		...
		"4. Register every path you evaluate as a candidate with register_optimization_candidate. This includes baseline/runtime-only candidates such as baseline, torch-compile, awq, fp8, nvfp4, or any custom backend you choose.",
		"5. Choose the backend and workflow yourself. Do not assume Triton, CuTe, or CUDA helpers exist. Use generic file tools plus run_command to create, edit, build, verify, and benchmark code.",
		"6. When a command belongs to a candidate stage, call run_command with session, candidate, and stage so Fusion persists the artifact. Use run_benchmark and run_profile with session and candidate for benchmark/profile stages.",
		"7. If compile, correctness, inference, or performance problems appear, inspect the outputs, patch the code or scripts, and retry. Do not stop at the first error unless the environment is fundamentally missing a dependency.",
		"8. Verify correctness before claiming success. Prefer explicit tolerances and benchmark evidence.",
		"9. Keep the optimization session state accurate by recording stages and using the candidate workspace instead of ad hoc temp paths.",
		"10. End with the current best candidate, what changed, what passed, what failed, and the next most valuable experiment if more time remains.",
		"",
		"Stage guidance",
