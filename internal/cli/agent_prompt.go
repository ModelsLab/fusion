...
func buildOptimizeRunPrompt(session *optimize.Session) string {
	...
	lines := []string{
		...
		"7. Exhaust the low-hanging search ladder before custom kernels: baseline -> packaged model-family, checkpoint, or runtime flavor variants -> runtime flags or attention implementation -> dtype or quant or checkpoint variants, including synthesized FP8 conversion when no packaged FP8 artifact exists -> torch.compile or CUDA graphs if supported -> Triton or CuTe or CUDA kernels -> deeper runtime patching.",
		"8. Choose the backend and workflow yourself. Do not assume Triton, CuTe, or CUDA helpers exist. Use generic file tools plus run_command to create, edit, build, verify, and benchmark code.",
		"9. When a command belongs to a candidate stage, call run_command with session, candidate, and stage so Fusion persists the artifact. Use run_benchmark and run_profile with session and candidate for benchmark/profile stages.",
		"10. For modality-specific workloads, create a generic harness manifest with create_harness_manifest and evaluate repeated benchmark samples plus quality metrics with assess_harness. Do not force image, video, or audio workloads into tokens/sec.",
		"11. After profile collection, use analyze_profile so Fusion converts raw Nsight output into a BottleneckReport and Prescription before you decide on deeper kernel changes.",
		"12. Use infer_hotspots to map kernels back to generic model stages like attention, transformer, unet, vae, scheduler, or upscaler when the workload is not a simple text decode loop.",
		"13. Use show_outer_loop_status and record_loop_decision to make the outer-loop state explicit. Do not launch deeper custom kernel search until packaged model, runtime, quantization, compile, and attention-backend branches are exhausted or explicitly blocked.",
		"14. During kernel search, persist round artifacts with save_round_artifact or record_reflexion under candidates/<id>/rounds/<n> so prompt, diagnosis, prescription, verify, bench, and reflexion data survive across turns.",
		"15. Use assess_benchmark_runs before ranking performance-sensitive candidates, and use rank_search_candidates to keep a top-K survivor set and promote the current best candidate explicitly.",
		"16. Write session memory with write_session_memory after important wins, failures, blockers, or environment changes so later turns can resume from markdown evidence instead of rediscovering context.",
		"17. If compile, correctness, inference, or performance problems appear, inspect the outputs, patch the code or scripts, and retry. Do not stop at the first fixable error or the first small performance win.",
		"18. Verify correctness before claiming success. Prefer explicit tolerances, reproducible seeds, benchmark evidence, and quality checks matched to the task.",
		"19. Keep the optimization session state accurate by recording stages and using the candidate workspace instead of ad hoc temp paths.",
		"20. For FP8 or other converted quantization paths, save the calibration recipe, runtime flags, and any fallback higher-precision modules. Compare normalized steady-state metrics, not just raw wall time. Choose metrics that fit the task: tokens/sec for text, images/sec for image generation, frames/sec or clips/hour for video, and rtf or x_real_time for audio.",
		"21. Maintain a current best candidate. If a new candidate regresses or breaks correctness, fall back to the current best and continue the search.",
		"22. End only after each applicable candidate family has been tested, rejected with evidence, or blocked by the environment. Then report the best candidate, what changed, what passed, what failed, and the next most valuable experiment if more time remains.",
		"",
		"Stage guidance",
