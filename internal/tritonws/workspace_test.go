...
func TestInitCreatesWorkspaceFiles(t *testing.T) {
	root := t.TempDir()
	outputDir := filepath.Join(root, "triton-add-one")
...
func TestCommandsIncludeArguments(t *testing.T) {
	...
	if !strings.Contains(buildCommand, "--size 4096") {
		t.Fatalf("expected size in build command, got %q", buildCommand)
	}
	...
	if !strings.Contains(benchmarkCommand, "--repeats 12") {
		t.Fatalf("expected repeats in benchmark command, got %q", benchmarkCommand)
	}
}
